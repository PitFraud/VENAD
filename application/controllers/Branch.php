...
class Branch extends MY_Controller {
	...
	public function add(){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			/*if(!empty($_FILES['president_img']['name'])){
                $config['upload_path'] = 'uploads/';
                $config['allowed_types'] = 'jpg|jpeg|png|gif';
                $config['file_name'] = $_FILES['president_img']['name'];
                $pic = $_FILES['president_img']['name'];
                //Load upload library and initialize configuration
                $this->load->library('upload',$config);
                $this->upload->initialize($config);
                if($this->upload->do_upload('president_img')){
                    $uploadData = $this->upload->data();
                    $president_img = $uploadData['file_name'];
                }else{
                    $president_img = '';
                }
			}else{
					if($branch_id)
					{
						$president_img = $this->input->post('president_img1');
					}
					else{
						$president_img ='Not uploaded';
					}
				 } */
			$data = array(

						//'unit_gid' => $this->input->post('unit_gid'),
