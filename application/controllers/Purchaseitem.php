...
class Purchaseitem extends MY_Controller
{
	...
	public function add(){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if (isset($shopid[0]->shop_id_fk)) {
				$shid = $shopid[0]->shop_id_fk;
			} else {
				$shid = 0;
			}
			/*-------------Dynamic Contents-------------*/
	...
	public function edit()
	{
		...
		if (isset($taxamount[0]->taxamount)) {
			$tax = $taxamount[0]->taxamount;
		} else {
			$tax = 0;
		}
...
// ###################################################################################################
	public function getproductcode(){
		$result = $this->Purchase_model->getproductcode();
		echo json_encode($result);
	}
	public function getsalesproductcode(){
		$result = $this->Purchase_model->getsalesproductcode();
		echo json_encode($result);
	}
	public function gethsncodes(){
		$result = $this->Purchase_model->gethsncodes();
		echo json_encode($result);
	}
	public function getSingleHSNDetails(){
		$id=$_POST['id'];
		$result=$this->Purchase_model->get_single_hsn_details($id);
		echo json_encode($result);
	}
	// ##############################################################################################
	...
	public function masterStock()
	{
		...
		for ($i = 0; $i < count($records); $i++) {
			...
			$nwstk = $stok[$i][0]->product_stock + $records[$i]->purchase_quantity;
			$updateData = array('product_stock' => $nwstk);
			$data = $this->General_model->update('tbl_product', $updateData, 'product_id', $records[$i]->product_id_fk);
		}
	...
	public function getItemName()
	{
		...
		$data = $this->Purchase_model->getItemsNames($item_code);
		echo json_encode($data);
	}
	...
	public function addnewItem()
	{
		...
		if(!empty($product_name)){
			$insert_array = [
				'product_type' => $this->input->post('prod_type_id'),
				'product_sub_type' => 0,
				'product_name' => $this->input->post('prod_name'),
				'product_code' => $this->input->post('prod_code'),
				'product_unit' => $this->input->post('prod_unit'),
				'product_open_stock' => $this->input->post('prod_op_stck'),
				'product_stock' => $this->input->post('prod_op_stck'),
				'min_stock' => $this->input->post('prod_min_stck'),
				'product_des' => $this->input->post('prod_desc'),
				'product_status' => 1,
				'product_created_date' => date('Y-m-d h:i:sa'),
				'product_updated_date' => date('Y-m-d h:i:sa'),
			];
			$result = $this->General_model->add('tbl_product',$insert_array);

			if($result)
			{
				$response_text = 'New Item added Succesfully';
