...
function addMore() {
	$("<DIV>").load("", function() {
		...
		var htmlVal = '<DIV class="product-item box box-success id="list"><input type="checkbox" name="item_index[]"/><table class="table table-bordered" cellspacing="2" ><tr><div class="form-group"><div class="col-sm-2"><b>Product</b><select name="product_id_fk[]" class="form-control product_num"  id="product_num'+counter+'" data-pms-required="true" autofocus /></select></div><div class="form-group"><div class="col-sm-1"><b><font color="red">Qty</font></b><input type="text"   data-validation="digitsOnly" data-pms-required="true" class="form-control quantity" id="pquantity_'+counter+'" name="purchase_quantity[]" placeholder="Qty"></div><div class="col-sm-1"><b>Unit Price</b><input type="text" data-pms-required="true" data-validation="digitsOnly" class="form-control price" id="sprice_'+counter+'" name="selling_price[]" placeholder="Selling Price"></div><div class="col-sm-1"><b>Discount</b><input type="text" data-pms-required="true" data-validation="digitsOnly" class="form-control discount" id="discount_'+counter+'" name="discount_price[]" placeholder="Discount Price"></div><div class="col-sm-1"><b>Tax</b><select class="form-control amountclass" data-pms-required="true" id="taxtype_'+counter+'" name="taxtype[]"></select></div><div class="col-sm-1"><div class="col-sm"><label>Total</label><span id="totalAmoun_'+counter+'"></span></label><input type="text" class="totalPrice"  name="purchase_total_price[]" id="total_price_'+counter+'" ><input type="hidden" id="taxpercantage_'+counter+'" ></div></div></tr></table></DIV>';
         ...
         $.ajax({
            ...
            success:function(data){
                ...
                for (var i=0;i<data.length;i++) {
                    ...
                    $('#sprice_'+counter+'').val(data[i].production_price); 
