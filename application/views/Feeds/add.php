                  ...
                  <div class="col-md-6">
                    ...
                    <select class="form-control" name="allotment">
                      <?php foreach ($allotment_details as $allotment): ?>
                        <option <?php if(isset($records->feeds_allotment_fk)){if($records->feeds_allotment_fk==$allotment->allot_id){echo "selected";}} ?> value="<?php echo $allotment->allot_id; ?>"><?php echo $allotment->member_name." - ".$allotment->product_name." - ".$allotment->allot_weight." ".$allotment->unit_name." - ".$allotment->allotment_date; ?></option>
                      <?php endforeach; ?>
                    </select>
                  </div>
                  <div class="col-md-6">
                    <label class="fsize">Distribution Date</label>
                    <input type="date" class="form-control" name="distribution_date" value="<?php if(isset($records->feeds_distribution_date)){ echo $records->feeds_distribution_date; } ?>">
                  </div>
                  <div class="col-md-6">
                    <label class="fsize">Feed name</label>
                    <select class="form-control" name="feeds_name">
                      <?php foreach ($feed_names as $feed): ?>
                        <option <?php if(isset($records->feeds_id_fk)){if($records->feeds_id_fk==$feed->feed_id){echo "selected";}} ?> value="<?php echo $feed->feed_id; ?>"><?php echo $feed->feed_name; ?></option>
                      <?php endforeach; ?>
                    </select>
                  </div>
                  <div class="col-md-6">
                  ...
                  <div class="col-md-6">
                    ...
                    <select class="form-control" name="unit">
                      <?php foreach ($units as $unit): ?>
                        <option <?php if(isset($records->feeds_unit)){if($records->feeds_unit==$unit->unit_id){echo "selected";}} ?> value="<?php echo $unit->unit_id ?>"><?php echo $unit->unit_name; ?></option>
                      <?php endforeach; ?>
                    </select>
                  </div>
                  <!-- ################################################# -->
