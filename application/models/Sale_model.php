...
class Sale_model extends CI_Model{
	...
	public function getSaleReport($param){
		...
		$this->db->select('*,tbl_member.*,tbl_member_type.*,COUNT(invoice_number) as slcount,SUM(total_price) as total,DATE_FORMAT(sale_date,\'%d/%m/%Y\') as sale_date');
	...
	{
		$this->db->select('*,((sale_price-((sale_price*100)/(100+taxamount)))/2)*sale_quantity as sgst, (taxamount/2) as taxper,((sale_price*100)/(100+taxamount)) as rate');
		$this->db->from('tbl_sale');
		$this->db->where('sale_status',1);
		$this->db->join('tbl_production_itemlist','tbl_production_itemlist.item_id = tbl_sale.product_id_fk','left');
	//	$this->db->join('tbl_shop','tbl_shop.shpid = tbl_sale.shop_id_fk','left');
