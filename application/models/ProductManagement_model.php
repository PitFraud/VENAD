...
class ProductManagement_model extends CI_Model{
	...
	public function getStockslist($item_id)
	{
		$this->db->select('item_name,item_quantity');
		$this->db->from('tbl_production_itemlist');
		$this->db->where('item_status',1);
		$this->db->where('item_id',$item_id);
		$query = $this->db->get();
	...
	public function getChickenTypeWeightList($chicken_type)
	{
		...
		$this->db->where('integration_stock_type',$chicken_type);
		$query = $this->db->get();
		return $query->row();
	}
